...
export const getMasterData = async (req, res) => {
    try {
      ...
      // Fetch booth data
      const [boothData] = await pool.query('SELECT * FROM booth_data WHERE year = ?',[year]);
      ...
      const formattedBoothData = boothData.map((booth) => {
        ...
        const boothCandidates = candidateData.filter((candidate) => candidate.booth_number === booth.booth_number);
